...
string two_letter_decryption(string word)
{
...
string playfair_decryption(string line)
{
	string word = "";
	string new_line = "";
	...
	for (int i = 0; i < line.size(); i++){
		...
		if (word.length() == 2){
			...
			if (check_J(word[0]) == 1 && check_J(word[1]) == 1){
				new_line = new_line + two_letter_decryption(word) + " ";
			} else {
...
int main()
{
	...
	ifstream myfile("encrypted_output_playfair.txt");
	ofstream myfile2("decrypted_output_playfair.txt");

	if (myfile.is_open() && myfile2.is_open()){
		
		while (getline(myfile, line)){
			string decrypted_line = playfair_decryption(line);
